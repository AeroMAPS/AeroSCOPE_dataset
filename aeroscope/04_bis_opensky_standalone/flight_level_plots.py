...
def flights_map_plot(flights_gpb_df, value_watched_flights):
    ...
    if 'n_flights' in flights_gpb_df.columns:
        airport_df = flights_gpb_df.groupby('dest').agg({
            'co2': 'sum',
            'ask': 'sum',
            'seats': 'sum',
            'n_flights': 'sum',
...
def flights_treemap_plot(flights_df, value_watched_flights):
    fig = px.treemap(flights_df,
                     path=[px.Constant("Total currently selected"), 'origin', 'dest', 'airline_iata',
                           'acft_icao'],
...
def distance_cumul_plot_flights(flights_df):
    ...
    sns.histplot(flights_df, x='distance_km', weights='n_flights', label= 'Flights', element='poly',fill=False, cumulative = True, stat='percent', ax=ax,bins=range(0, int(flights_df["distance_km"].max()) + 50, 50),)
