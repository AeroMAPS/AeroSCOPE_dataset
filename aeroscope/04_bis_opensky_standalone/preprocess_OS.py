...
def preprocess():
    ...
    load_factor=0.83
    flights_df = pd.read_csv('../04_bis_opensky_standalone/data/os_alone_test.csv', keep_default_na=False, na_values=['', 'NaN'],
    ...
    continental_flows = flights_df_conti.groupby(
        ...
        ['seats', 'ask', 'fuel_burn', 'co2','n_flights', 'CO2 (Mt)', 'ASK (Bn)', 'Seats (Mn)']].sum().reset_index()
    ...
    country_flows = flights_df.groupby(
        ['departure_country', 'arrival_country', 'departure_country_name', 'arrival_country_name', 'departure_ISO3',
         'arrival_ISO3'])[['seats', 'ask', 'fuel_burn', 'co2', 'n_flights', 'CO2 (Mt)', 'ASK (Bn)', 'Seats (Mn)']].sum().reset_index()
    ...
    continental_flows.to_csv('./plot_files_os/continental_flows.csv')
    continental_flows_non_dir.to_csv('./plot_files_os/continental_flows_non_dir.csv')
    conti_scatter.to_csv('./plot_files_os/conti_scatter.csv')
    flights_df_conti.to_csv('./plot_files_os/flights_df_conti.zip',compression='zip')

    #save country level data
    country_flows.to_csv('./plot_files_os/country_flows.csv')
    country_fixed.to_csv('./plot_files_os/country_fixed.csv')

    #save flight_level_data
    flights_df.to_csv('./plot_files_os/flights_df.zip',compression='zip')
