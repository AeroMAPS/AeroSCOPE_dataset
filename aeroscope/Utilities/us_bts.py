...
class FlightBts(FlightData):
    ...
    def preprocess_data(self, keep_time=False):
        ...
        data_path = os.path.join(
            root_path, "03_routes_schedule", "data/BTS/aircraft_codes_icao_joined.csv"
        )
        ...
        print(
            "Size of df after adding aircraft info {}, number of flights: {}".format(
                len(self.df.index), self.df.departures_performed.sum()
            )
        )
        # standardizing column names
        self.df.rename(
            columns={
                "unique_carrier": "airline_iata",
                "ICAO_Code": "acft_icao",
                "YEAR": "year",
                "MONTH": "month",
                "departures_performed": "n_flights",
                "class": "flight_class",
            },
            inplace=True,
        )
        ...
        print(
            "Size of df after grouping {}; number flights {}".format(
                len(self.df.index), self.df.n_flights.sum()
            )
        )
        ...
        self.df["total_payload"] = (
            self.df["mail"] + self.df["freight"] + self.df["passengers"] * 100
        ) / self.df["n_flights"]
        ...
        print(
            "Size of df after airports process {}; number flights {}".format(
                len(self.df.index), self.df.n_flights.sum()
            )
        )
    ...
    def add_airports_coord(
        ...
        airport_path = os.path.join(
            root_path, "03_routes_schedule", "data/BTS/T_MASTER_CORD_unique.csv"
        )
        airports_df = pd.read_csv(airport_path, keep_default_na=False, sep=";")

        continents_path = os.path.join(
            root_path, "03_routes_schedule", "data/country_codes.csv"
        )
        continents = pd.read_csv(continents_path, keep_default_na=False)
        airports_df = airports_df.merge(
            continents[["Continent_Code", "Two_Letter_Country_Code"]],
            right_on="Two_Letter_Country_Code",
            left_on="AIRPORT_COUNTRY_CODE_ISO",
            how="left",
        )
        airports_df.rename(
            columns={
                "AIRPORT": "bts_code",
                "AIRPORT_COUNTRY_CODE_ISO": "iso_country",
        ...
        self.df = self.df.merge(
            airports_df[
                ["bts_code", "LONGITUDE", "LATITUDE", "iso_country", "continent"]
            ],
            left_on="origin",
            right_on="bts_code",
            how="left",
        ...
        self.df.drop(columns=["bts_code_x", "bts_code_y"], inplace=True)
