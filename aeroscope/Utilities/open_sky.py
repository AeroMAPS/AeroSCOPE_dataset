...
class FlightOpenSky(FlightData):
    ...
    def preprocess_data(self, keep_time=False):
        ...
        data_path = os.path.join(
            root_path,
            "03_routes_schedule",
            "data/open_sky/aircraft-database-complete-2022-05.csv",
        )
        ...
        ac_ref = ac_ref[
            ["icao24", "typecode", "operator_jz", "operatoricao"]
        ].drop_duplicates(subset="icao24", keep="last")
        self.df = pd.merge(
            self.df, ac_ref, left_on="icao24", right_on="icao24", how="left", sort=False
        )

        # we use ailine ICAO code, but it is named IATA in the following for compatibility reasons
        self.df.rename(
            columns={"operatoricao": "airline_iata", "typecode": "aircraft_type"},
            inplace=True,
        )
        ...
        data_path = os.path.join(
            root_path, "03_routes_schedule", "data/open_sky/aircraft_classification.csv"
        )
        ...
        data_path = os.path.join(
            root_path, "03_routes_schedule", "data/ourairports.csv"
        )
        ...
        self.df.drop(columns=["ident_x", "ident_y"], inplace=True)
    ...
    def sum_data(self):
        ...
        """
        self.df = self.df.groupby(
            ["airline_iata", "origin_iata", "dest_iata", "acft_icao"], as_index=False
        ).agg({"n_flights": "sum"})
