...
def continental_map_plot(conti_scatter, continental_flows_non_dir, value_watched_conti):
    ...
    if len(conti_scatter)>0:
        ...
        for i in range(len(continental_flows_non_dir)):
            fig.add_trace(
                go.Scattergeo(
                    ...
                    mode='lines',
                    line=dict(width=continental_flows_non_dir[value_watched_conti][i] / (
                                .02 * max(continental_flows_non_dir[value_watched_conti])), color='#EE9B00'),
                    opacity=1,
                    showlegend=False
                )
            )
        ...
        fig.add_trace(go.Scattergeo(
            ...
            mode='markers',
            marker=dict(
                size=conti_scatter[value_watched_conti] / (.0001 * max(conti_scatter[value_watched_conti])),
                sizemode='area',
